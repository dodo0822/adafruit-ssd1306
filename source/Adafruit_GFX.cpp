...
#include "adafruit-ssd1306/glcdfont.h"
...
void Adafruit_GFX::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) 
{
  int16_t steep = abs(y1 - y0) > abs(x1 - x0);
	...
	if (steep) 
	{
    _adafruit_swap(x0, y0);
    _adafruit_swap(x1, y1);
  }

  if (x0 > x1) 
	{
    _adafruit_swap(x0, x1);
    _adafruit_swap(y0, y1);
  }
...
void Adafruit_GFX::fillTriangle ( int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) 
{
  ...
  if (y0 > y1) 
	{
    _adafruit_swap(y0, y1); _adafruit_swap(x0, x1);
  }
  if (y1 > y2) 
	{
    _adafruit_swap(y2, y1); _adafruit_swap(x2, x1);
  }
  if (y0 > y1) 
	{
    _adafruit_swap(y0, y1); _adafruit_swap(x0, x1);
